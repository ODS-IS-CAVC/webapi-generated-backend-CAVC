...
public interface CarrierTransCapacityDelApi {
    ...
    /**
     * PUT /carrier_trans_capacity_del : 運送計画情報（明細型）を削除　暫定対応
     * 運送計画情報（明細型）を削除します。
     *
     * @param carrierTransCapacityDel 運送計画情報（明細型） (required)
     * @return 運送計画情報（明細型）を正常に削除しました。 (status code 201)
     *         or HTTP400エラー (status code 400)
     *         or HTTP500エラー (status code 500)
     */
    @Operation(
        operationId = "carrierTransCapacityDelPut",
    ...
    @RequestMapping(
        ...
        value = "/carrier_trans_capacity_del",
        consumes = { "application/json" }
    )
    
    ResponseEntity<Void> carrierTransCapacityDelPut(
        ...
        @Parameter(name = "CarrierTransCapacityDel", description = "運送計画情報（明細型）", required = true) @Valid @RequestBody CarrierTransCapacityDel carrierTransCapacityDel
    ); 
